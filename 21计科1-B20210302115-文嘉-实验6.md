...
```mermaid
flowchart LR
A(开始) --> B{初始化 new_lst 为 lst 中的第一个人}
B -->|遍历 lst 中的每个人| C{检查年龄是否大于 new_lst 中的第一个人}
C -->|年龄更大| D{用只包含当前人的列表替换 new_lst}
D --> E(结束)
C -->|年龄相等| F{将当前人添加到 new_lst}
F --> G(遍历下一个人)
G --> C
C -->|年龄更小| G
```
...
1. 什么是函数式编程范式？
```
  函数式编程范式是一种编程方法，它强调将计算过程看作是数学函数的求值，而不是改变状态的过程。这种范式强调函数的纯洁性、无副作用性和可组合性。
```
2. 什么是lambda函数？请举例说明。
```
 Lambda函数是一种匿名函数，它主要用于简化代码和模块化功能。Lambda函数可以接受任意数量的参数，并返回一个值。例如：
   ```python
   add = lambda x, y: x + y
   result = add(3, 4)
   print(result)  # 输出：7
...
```
高阶函数是指接受其他函数作为参数或返回值的函数。常用的高阶函数有：

   - 组合函数：将两个或多个函数组合在一起，以创建一个新的函数。例如：
      ...
      result1 = compose(add, multiply)(3, 4)  # 输出：14
      result2 = compose(multiply, add)(3, 4)  # 输出：18
      ```

   - 映射函数：对一个函数的输入应用另一个函数，然后返回结果。例如：
   ...
   - 过滤器函数：对一个函数的输入应用另一个函数，然后根据结果过滤输出。例如：
...
通过本次实验我学习Python函数的基本用法以及学习了lambda函数和高阶函数的使用，掌握函数式编程的概念和实践，让我更熟悉了Python语言的使用，提高了编程能力。