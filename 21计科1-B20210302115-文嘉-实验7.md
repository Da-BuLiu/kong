...
class Block:
    ...
    def get_surface_area(self):
        return 2 * (self.width * self.height + self.length * self.height + self.width * self.length)
```
...
graph LR
A(开始)
B(将draft赋值给self.draft)
C(将crew赋值给self.crew)
D(计算crew_weight = self.crew * 1.5)
E(判断self.draft - crew_weight是否大于20)
F[返回True]
G[返回False]
A --> B
B --> C
...
```
__init__方法是类的构造函数，当创建类的新实例时，它会自动被调用。它主要用于初始化对象的属性。
...
```
要继承父类，子在类中使用`super()`函数调用父类的构造函数。要改写父类的方法，子在类中重写父类的方法，使用`super()`函数调用父类的方法，并传入适当的参数。
...
print(dog.speak())  # 输出：Woof! I am a dog.
```

3. Python类有那些特殊的方法？它们的作用是什么？请举三个例子并编写简单的代码说明。
```
Python类中的特殊方法主要包括：
__init__：类的构造函数，用于初始化对象属性。
__str__：当使用`print()`函数打印对象时，会自动调用此方法生成对象的描述。
__eq__：用于判断两个对象是否相等。
...
print(circle1 == circle3)  # 输出：False
```
## 实验总结
  通过本次实验我学习了Python类和继承的基础知识以及namedtuple和DataClass的使用，提高了编程能力。
